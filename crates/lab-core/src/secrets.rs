...
const SECRETS_DIR: &str = ".lab";
const SECRETS_FILE: &str = "secrets.env";
const SECRETS_EXAMPLE: &str = "secrets.env.example";

/// Get the secrets file path.
pub fn secrets_file_path(workdir: &Path) -> PathBuf {
    workdir.join(SECRETS_DIR).join(SECRETS_FILE)
}

/// Load secrets from `.lab/secrets.env` (or a custom path).
...
pub fn pull_secrets_from_gitlab(workdir: &Path) -> Result<Variables> {
    ...
    if !result.skipped_hidden.is_empty() {
        warn!(
            ...
            "skipped hidden variables (value not available via API — add manually to .lab/secrets.env)"
        );
    }
...
/// Save variables to `.lab/secrets.env`.
pub fn save_secrets_file(workdir: &Path, vars: &Variables) -> Result<()> {
    let dir = workdir.join(SECRETS_DIR);
    ...
    let mut content = String::from("# Generated by `lab secrets pull` — DO NOT COMMIT\n");
    content.push_str("# Add .lab/secrets.env to your .gitignore\n\n");

    for (key, val) in vars {
        let value = val.value();
        // Escape newlines and quotes for env file format
        if value.contains('\n') || value.contains('"') {
            // For multiline values, use base64 encoding
    ...
    install_pre_commit_hook(workdir);

    info!(path = %path.display(), count = vars.len(), "saved secrets");
    Ok(())
}

/// Generate `.lab/secrets.env.example` from pipeline variable references.
...
pub fn generate_secrets_example(pipeline: &Pipeline, workdir: &Path) -> Result<()> {
    ...
    let path = dir.join(SECRETS_EXAMPLE);
    let mut content = String::from("# Required CI/CD variables for this pipeline\n");
    content.push_str("# Copy to .lab/secrets.env and fill in values\n");
    content.push_str("# Or run `lab secrets pull` to fetch from GitLab\n\n");
    ...
    // Also create empty secrets.env if it doesn't exist
    let secrets_path = dir.join(SECRETS_FILE);
    if !secrets_path.exists() {
        std::fs::write(&secrets_path, "# Add your secrets here (KEY=VALUE)\n").map_err(|e| {
            LabError::FileRead {
                path: secrets_path,
                source: e,
            }
...
/// Install a pre-commit hook that blocks `.lab/secrets.env` from being committed.
fn install_pre_commit_hook(workdir: &Path) {
    let hooks_dir = workdir.join(".git/hooks");
    if !hooks_dir.exists() {
        return;
    }

    let hook_path = hooks_dir.join("pre-commit");
    let lab_marker = "# lab-secrets-guard";
    let hook_script = r#"
# lab-secrets-guard
# Prevent .lab/secrets.env from being committed
...
"#;
    ...
    if hook_path.exists() {
        // Append to existing hook if our guard isn't already there
        if let Ok(existing) = std::fs::read_to_string(&hook_path) {
            if !existing.contains(lab_marker) {
                let _ = std::fs::write(&hook_path, format!("{existing}\n{hook_script}"));
                debug!("appended secrets guard to existing pre-commit hook");
            }
        }
    } else {
        let content = format!("#!/bin/sh\n{hook_script}");
        let _ = std::fs::write(&hook_path, content);
        // Make executable
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
        ...
        debug!("installed pre-commit hook for secrets protection");
    }
}

fn ensure_gitignore(workdir: &Path) {
    let gitignore_path = workdir.join(".gitignore");
    let entry = ".lab/secrets.env";
    if gitignore_path.exists() {
        if let Ok(existing) = std::fs::read_to_string(&gitignore_path) {
            if !existing.contains(entry) {
                let sep = if existing.ends_with('\n') { "" } else { "\n" };
                let _ = std::fs::write(&gitignore_path, format!("{existing}{sep}{entry}\n"));
            }
        }
    }
}
